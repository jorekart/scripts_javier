...
def parse_and_open_imas_entry():
    ...
    parser.add_argument('-uri', '--uri', help='URI of the data entry', required=False, default='none', type=str)
    parser.add_argument('-s', '--shot', help='Shot number', required=False, type=int)
    parser.add_argument('-r', '--run', help='Run number', required=False, type=int)
    ...
    if args['backend'] == 'MDSPLUS':
        backend_id = imas.ids_defs.MDSPLUS_BACKEND
    else:
        backend_id = imas.ids_defs.HDF5_BACKEND
