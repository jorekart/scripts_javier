...
program get_Edep_Idep
  ...
  real(8), allocatable :: T_max(:), melt_duration(:), E_dep(:), tri_areas(:), I_dep(:)
  logical :: valid
  ...
  do i_step = i_begin, i_end, i_jump_steps
    ...
    if (i_step==i_begin) then
      allocate(T_max(n_tri), melt_duration(n_tri), E_dep(n_tri), I_dep(n_tri))
      allocate(tri_areas(n_tri))
      T_max = 0.d0; melt_duration = 0.d0; E_dep = 0.d0; I_dep = 0.d0
  ...
  open(unit=20, file="Edep_Idep.dat", status="unknown", action="write", iostat=ierr)
  ...
  do i=1, n_tri
    ...
    write(20, '(9ES14.6)') tri_phi, tri_theta, T_max(i), melt_duration(i), E_dep(i)*tri_areas(i), tri_areas(i), tri_R, tri_Z, I_dep(i)*tri_areas(i)
