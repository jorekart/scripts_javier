...
y = spec_heat_capacity_w(T) * rho_w * 1e-6

print(y[T > T_melt_w][:10])
print(np.isnan(y[T > T_melt_w]).all())

# Plot 1: specific heat capacity
plt.figure(figsize=(6, 5))
plt.plot(T, spec_heat_capacity_w(T)*rho_w*1e-6, label="W", linewidth=2.5)
plt.plot(T, spec_heat_capacity_be(T)*rho_Be*1e-6, label="Be", linewidth=2.5)
...
plt.ylabel(r"$c_p \, \rho$ [J/($m^3$K)]", fontsize=16)
plt.title("Volumetric heat capacity vs temperature")
plt.grid(True, alpha=0.3)
plt.legend(frameon=True, fontsize=14)
#plt.yscale("log")
plt.tight_layout()
cp_path = "specific_heat_capacity_W_Be.png"
plt.savefig(cp_path, dpi=200, bbox_inches="tight")
plt.show()
plt.close()

# Plot 2: thermal conductivity
plt.figure(figsize=(6, 5))
plt.plot(T, heat_conductivity_w(T), label="W", linewidth=2.5)
plt.plot(T, heat_conductivity_be(T), label="Be", linewidth=2.5)
plt.axvline(T_melt_w, linestyle="--", linewidth=2, alpha=0.8, label=r"W melt", color='lightblue')
plt.axvline(T_melt_be, linestyle="--", linewidth=2, alpha=0.8, label=r"Be melt", color='orange')
plt.xlabel("Temperature [K]", fontsize=16)
plt.ylabel("K [W/(m·K)]", fontsize=14)
plt.title("Thermal conductivity vs temperature")
plt.grid(True, alpha=0.3)
plt.legend(frameon=True, fontsize=14)
plt.tight_layout()
k_path = "thermal_conductivity_W_Be.png"
